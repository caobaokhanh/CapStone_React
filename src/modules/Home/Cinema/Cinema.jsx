...
import { ButtonCinema, Item } from "../../Details/Showtimes/styledShowItem";

import {
  getLogoCinema,
  getInformationCinemas,
  getCinemaShowTimes,
} from "../../../apis/cinemaAPI";
...
export default function Cinema({ cinemaId }) {
  ...
  const { data: cinemaSystem = [], isLoading } = useQuery({
    ...
    queryFn: () => getLogoCinema(cinemaId),
  });

  const handleChangeTab = async (cinemaSystemsId) => {
    try {
      const infoCinema = await getInformationCinemas(cinemaSystemsId);
